...
class DreamDenoiser(Denoiser):
    ...
    def __init__(self, device = 'cuda', save_to_hf_cache = True):
        ...
        self.name = "Dream-Coder-v0-Instruct-7B"
        self.model = AutoModel.from_pretrained("Dream-org/"+self.name, cache_dir = hf_cache_dir, trust_remote_code=True, torch_dtype=torch.bfloat16).to(device).eval()
        self.tokenizer = AutoTokenizer.from_pretrained("Dream-org/"+self.name, cache_dir = hf_cache_dir, trust_remote_code=True)
    ...
    def __call__(self, input_seq):
        ...
        # shift logits and ignore expand token
